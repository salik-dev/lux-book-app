...
import { EventBullet } from "@/pages/admin/schedule/calendar/components/month-view/event-bullet";
import { DroppableDayCell } from "@/pages/admin/schedule/calendar/components/dnd/droppable-day-cell";
import { MonthEventBadge } from "@/pages/admin/schedule/calendar/components/month-view/month-event-badge";

import { cn } from "@/pages/admin/schedule/lib/utils";
import { getMonthCellEvents } from "@/pages/admin/schedule/calendar/helpers";

import type { ICalendarCell, IEvent } from "@/pages/admin/schedule/calendar/interfaces";
    ...
    <DroppableDayCell cell={cell}>
      <div className={cn("flex h-full flex-col gap-1 border-l border-t py-1.5 lg:pb-2 lg:pt-1", isSunday && "border-l-0")}>
        <button
          onClick={handleClick}
          className={cn(
            "flex size-6 translate-x-1 items-center justify-center rounded-md text-xs font-semibold hover:bg-accent focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring lg:px-2",
