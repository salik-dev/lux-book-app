...
-- Remove foreign key constraint to allow admin users without auth accounts
...
INSERT INTO public.app_settings (id, key, value, description) VALUES
  (gen_random_uuid(), 'company_name', '"Fjord Fleet"', 'Company display name'),
  (gen_random_uuid(), 'default_currency', '"NOK"', 'Default currency for pricing'),
  (gen_random_uuid(), 'vat_rate', '0.25', 'VAT rate (25%)'),
  (gen_random_uuid(), 'booking_lead_time_hours', '2', 'Minimum hours before pickup'),
  (gen_random_uuid(), 'max_booking_days', '30', 'Maximum booking duration in days'),
  (gen_random_uuid(), 'email_notifications', 'true', 'Enable email notifications'),
  (gen_random_uuid(), 'maintenance_mode', 'false', 'System maintenance mode');